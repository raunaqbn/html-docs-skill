...
async function courseCommand(input: string[]) {
    ...
    const project = courseProjectSchema.parse({
      ...
      modules: [{
        ...
        lessons: [{
          ...
          checks: [
            { id: 'check-one', prompt: 'Which statement best captures the central mechanism?', choices: ['The source-grounded explanation', 'An unrelated claim'], answer: 0, explanation: 'The first choice is tied to the lesson evidence.', evidenceIds: [evidenceId] },
            { id: 'check-two', prompt: 'What should you consult when applying the model?', choices: ['The cited source evidence', 'An unsupported assumption'], answer: 0, explanation: 'Course claims remain traceable to the captured evidence.', evidenceIds: [evidenceId] },
          ],
          status: 'planned',
        }],
      }],
  ...
  <p class="note">Replace this scaffold with the lesson’s evidence-grounded explanatory diagram.</p>
    ...
    await Promise.all([
      ...
      writeFile(join(output, 'COURSE.md'), `# ${title}\n\nAudience: ${project.audience}\n\nThis file is the human-readable course brief. The active agent should replace the scaffold lesson with a source-grounded module map before publishing.\n`),
